...
TRAIT_LANDMARK_MAPPING = {
    "Chin_34_35": [6, 7, 8, 9, 10],
    "Chin_34_36": [5, 6, 7, 8, 9, 10, 11],
    "Chin_35_36": [4, 5, 6, 7, 8, 9, 10, 11, 12],
    "Lowercheek_37_38": [2, 3, 4, 12, 13, 14],
}

CHIN_HORIZONTAL_WEIGHTS = {
    4: 1.00,
    5: 0.85,
    6: 0.65,
    7: 0.35,
...
def find_canonical_trait_name(trait_key: str) -> str | None:
    for canonical_trait in TRAIT_LANDMARK_MAPPING:
        if trait_key == canonical_trait or trait_key.endswith(f"_{canonical_trait}"):
            return canonical_trait
    return None
    ...
    for trait_key, raw_record in payload.items():
        ...
        mapped_traits.append(
            TraitRecord(
                ...
                landmarks=list(TRAIT_LANDMARK_MAPPING[canonical_trait]),
            )
        )
...
def horizontal_sign(landmark_id: int) -> float:
    if landmark_id < 8:
        return -1.0
    if landmark_id > 8:
        return 1.0
...
def apply_trait_displacement(
    ...
    if trait.canonical_trait.startswith("Chin_"):
        for landmark_id in trait.landmarks:
            horizontal_weight = CHIN_HORIZONTAL_WEIGHTS.get(landmark_id, 0.0)
            vertical_weight = CHIN_VERTICAL_WEIGHTS.get(landmark_id, 0.0)
            displacement_map[landmark_id]["dx"] += (
                horizontal_sign(landmark_id) * horizontal_weight * trait.displacement_pixels
            )
            displacement_map[landmark_id]["dy"] += vertical_weight * trait.displacement_pixels
        return

    if trait.canonical_trait == "Lowercheek_37_38":
        for landmark_id in trait.landmarks:
            horizontal_weight = LOWERCHEEK_HORIZONTAL_WEIGHTS.get(landmark_id, 0.0)
            displacement_map[landmark_id]["dx"] += (
                horizontal_sign(landmark_id) * horizontal_weight * trait.displacement_pixels
            )
        return

    raise ValueError(f"Unsupported canonical trait mapping: {trait.canonical_trait}")
...
def build_displacements(
    ...
    displacement_map: dict[int, dict[str, float]] = {
        landmark_id: {"dx": 0.0, "dy": 0.0} for landmark_id in landmarks
    }
    for trait in traits:
        apply_trait_displacement(trait, displacement_map)
